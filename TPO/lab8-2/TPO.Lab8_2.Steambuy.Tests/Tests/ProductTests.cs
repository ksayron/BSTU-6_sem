...
public sealed class ProductTests : BaseTest
{
    ...
    public void ProductPage_ShouldDisplayKeyInformation()
    {
        ...
        {
            Assert.Ignore("Product title is unavailable in current runtime state.");
        }

        Assert.That(title, Is.Not.Empty, "Product title is missing.");
        if (!productPage.HasPriceLikeValue() || !productPage.HasKeyDetails())
        {
            Assert.Ignore("Product detail blocks are unavailable in current runtime state.");
        }
